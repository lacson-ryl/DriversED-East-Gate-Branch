...
app.use((req, res, next) => {
  if (
    req.originalUrl.startsWith("/account/user-login") ||
    req.originalUrl.startsWith("/auth/google/user")
  ) {
    req.sessionName = "userSession";
  } else if (
    req.originalUrl.startsWith("/account/adminlogin") ||
  ...
  (req, res, next) => {
    adminPassport.authenticate("google", (err, user, info) => {
      if (err) {
        ...
        return res.redirect("/account/adminlogin?error=Internal Server Error");
      }
      if (!user) {
      ...
      req.login(user, (loginErr) => {
        if (loginErr) {
          ...
          return res.redirect("/account/adminlogin?error=Login failed.");
        }
